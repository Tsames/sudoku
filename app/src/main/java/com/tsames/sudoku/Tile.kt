...
class Tile (val id: Int, val square: Int, val column:Int, val row:Int) {
    ...
    fun pickValue(): Int? {
        ...
        if (value != 0) {
            println("Value is already set: $value. Cannot pick a value while one is already assigned.")
            return null
        }
        ...
        println("Selected $value as a value for Tile[$id] @ position $column in row $row")
    ...
    override fun toString(): String {
        return "Tile[$id] @ Square: $square / Row: $row / Column: $column -- Value: $value"
    }
}
